...
interface Dao {
    ...
    @Query("SELECT * FROM items") // выводит все записи со всех карт
    fun getAllItemsWithAllCards(): Flow<List<Items>>

    @Query("SELECT * FROM items WHERE data BETWEEN :dateFrom AND :dateTo") // выводит записи по выбранной дате со всех карт
    fun getItemsByDateWithAllCards(dateFrom: String, dateTo: String): Flow<List<Items>>

    @Query("SELECT * FROM items WHERE card = :selectedCard AND data BETWEEN :dateFrom AND :dateTo") // выводит записи по выбранной дате с выбранной картой
    fun getItemsByDateWithSelectedCard(dateFrom: String, dateTo: String, selectedCard: String): Flow<List<Items>>

    @Query("SELECT * FROM items WHERE card = :selectedCard") // выводит записи за все время с выбранной картой
